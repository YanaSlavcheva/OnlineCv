$(document).ready(function () {
    ...
    // education template
    var educationTemplate = "<div class=\"col-xs-12 col-md-12 col-lg-12\"><h2>{{heading}}</h2>{{#items}}<article class=\"row smallArticle\"><div class=\"col-sm-12 col-md-4 col-lg-4\"><p class=\"custom-header\">{{date}}</p></div><div class=\"col-sm-12 col-md-8 col-lg-8\"><h4>{{university}}</h4><h5>{{title}}</h5><p>{{description}}</p>{{#links}}<p><a href=\"{{url}}\" target=\"_blank\">{{linkName}}</a></p>{{/links}}</div></article>{{/items}}</div>";
